...
2. **Execute** — `jobs/execute.py` prepares and routes jobs (clone URLs, branch resolution); `jobs/runner/process.py` wraps with error handling and queue management.
...
│   ├── webhook/         # Webhook server (server.py), helpers, mention extraction, K8s entrypoint
...
├── jobs/                # Job payload, execution, runner and queue subpackages
