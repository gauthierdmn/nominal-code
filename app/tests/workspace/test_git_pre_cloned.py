...
class TestExternallyManagedWorkspace:
    ...
    async def test_ensure_ready_skips_when_no_clone_url_and_git_exists(
        ...
        with (
            patch.object(
                workspace_no_clone_url, "_clone", new_callable=AsyncMock,
            ) as mock_clone,
            patch.object(
                workspace_no_clone_url, "_update", new_callable=AsyncMock,
            ) as mock_update,
        ):
...
class TestManagedWorkspace:
    ...
    async def test_ensure_ready_clones_when_clone_url_set_and_no_git(
        self, workspace_with_clone_url,
    ):
        with patch.object(
            workspace_with_clone_url, "_clone", new_callable=AsyncMock,
        ) as mock_clone:
    ...
    async def test_ensure_ready_updates_when_clone_url_set_and_git_exists(
        ...
        with patch.object(
            workspace_with_clone_url, "_update", new_callable=AsyncMock,
        ) as mock_update:
            await workspace_with_clone_url.ensure_ready()
