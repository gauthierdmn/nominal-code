...
def redact_url(url: str) -> str:
    ...
    Matches the ``https://user:TOKEN@host`` pattern and replaces the
    token portion.

    Args:
        url (str): A URL or string that may contain an embedded token.

    Returns:
        str: The string with token portions replaced by ``***``.
    """

    return URL_TOKEN_PATTERN.sub(r"\1***\2", url)
