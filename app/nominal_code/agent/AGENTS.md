...
`cost.py` tracks token usage and computes dollar costs for each agent invocation.

- **`_get_pricing()`** — lazily loads the bundled `data/pricing.json` file (cached via `lru_cache`). The JSON is generated by `scripts/update_pricing.py` from the LiteLLM community pricing database, updated weekly via a GitHub Actions workflow.
- **`CostSummary`** — frozen dataclass with accumulated token counts (`input`, `output`, `cache_creation`, `cache_read`), total cost in USD, provider, model, and API call count.
- **`compute_cost(usage, model)`** — looks up model pricing and returns dollar cost, or `None` for unknown models.
- **`build_cost_summary()`** — builds a `CostSummary` from accumulated `TokenUsage`.

Both runners populate `AgentResult.cost`:
- **API runner** — accumulates `TokenUsage` from each `LLMResponse.usage` across the agentic loop, then calls `build_cost_summary()`.
- **CLI runner** — extracts `total_cost_usd` and `usage` dict from the SDK's `ResultMessage`.

Cost is logged by the review handler and formatted for CI output by `ci.py._format_cost_summary()`.
...
├── cost.py          # CostSummary, pricing loader, compute_cost(), build_cost_summary()
├── data/
│   └── pricing.json # Bundled model pricing (auto-updated from LiteLLM via scripts/update_pricing.py)
