...
1. **API rate limiting / throttling** — `http.py` retries on 502/503/504 but ignores 429 (rate limit) responses entirely. No adaptive backoff, no jitter, no token bucket. 100 concurrent PRs × ~20 API calls each = 2000 calls in seconds, instantly hitting GitHub's 5000/hr limit.

2. **Wire RedisConversationStore into webhook/K8s mode** — `main.py` always creates `MemoryConversationStore`, even with the Kubernetes runner. K8s job pods start fresh with no conversation context, so multi-turn is broken. `RedisConversationStore` exists but isn't used.

3. **Horizontal scaling** — single webhook server replica (`replicas: 1`), no `HorizontalPodAutoscaler`, no `PodDisruptionBudget`. Single point of failure.

4. **Job concurrency limits** — `AsyncioJobQueue` spawns unlimited consumer tasks per PR key with no cap. `KubernetesRunner` creates unlimited K8s Jobs in parallel. No semaphore or configurable limit anywhere in `config.py`.

5. **Workspace disk pressure** — each PR gets a shallow clone in ephemeral storage (`/tmp/nominal-code`). 100 repos × 10 PRs × ~50MB = 50GB, exceeds typical K8s ephemeral storage. No `PersistentVolume` defined. Cleanup runs every 6 hours so stale workspaces accumulate.

6. **Redis persistence and sizing** — no `PersistentVolume`, 128Mi memory limit. Pod restart loses all queue state and pub/sub channels. 500+ concurrent PRs pushes close to the memory limit.

7. **GitHub App token handling** — single shared `GitHubAppAuth` instance with mutable `installation_id`. Causes cache thrashing and a race condition under multi-installation traffic. See `docs/scaling-github-app-auth.md` for full analysis and proposed fix.

### Important

8. **Per-repo configuration** — same agent model, max turns, and guidelines for every repo. No way to assign different bots or settings per org/repo.

9. **Circuit breaker** — if the LLM provider or GitHub API is down, jobs keep failing with no backoff. `backoffLimit: 0` on K8s Jobs means no retry, but the server doesn't stop submitting new ones either.

10. **K8s job completion reliability** — fire-and-forget: if the Redis pub/sub completion message is missed (e.g. server restart during job execution), the server never learns the job finished. `ttlSecondsAfterFinished: 3600` leaves finished pods around for an hour with no reconciliation.
