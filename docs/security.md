...
When running review jobs on Kubernetes, the job runner applies security hardening to pod specs by default (`pod_security_enabled=True` in `KubernetesConfig`).
...
| `runAsUser` | `1000` | Configurable UID |
...
All security settings are configurable via `KubernetesConfig`:

| Field | Default | Description |
|-------|---------|-------------|
| `pod_security_enabled` | `true` | Enable/disable all pod security hardening |
| `run_as_user` | `1000` | UID for the container process |
| `read_only_root_filesystem` | `true` | Mount root filesystem as read-only |
| `automount_service_account_token` | `false` | Mount service account token in pods |

Set `pod_security_enabled=False` to disable all security context settings (not recommended for production).
